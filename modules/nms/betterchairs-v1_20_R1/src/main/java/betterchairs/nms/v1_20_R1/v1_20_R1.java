package betterchairs.nms.v1_20_R1;
...
public class v1_20_R1 extends ChairNMS {
    @Override
    public @NotNull org.bukkit.entity.Entity spawnChairEntity(@NotNull Location loc, int regenerationAmplifier, boolean useArmorStand) {
        ...
        if (useArmorStand) {
            nmsEntity = new CustomArmorStand(nmsWorld, loc.getX(), loc.getY() - .15, loc.getZ(), regenerationAmplifier);
        } else {
            nmsEntity = new CustomArrow(nmsWorld, loc.getX(), loc.getY() + .25, loc.getZ(), regenerationAmplifier);
        }
